...
CHUNK_SIZE    = 2000
EMAIL_REGEX   = r"(?<![A-Z0-9])[A-Z]{5}[0-9]{4}[A-Z](?![A-Z0-9])"
OUTPUT_DOCX   = "final_output_pan.docx"
OUTPUT_XLSX   = "final_output_pan.xlsx"
TEMP_DIR      = "temp_parts"
...
def process_chunk(args):
    ...
    for identifier, ts, payload in chunk:
        ...
        for mt in mlist:
            ...
            # find field name
            path = find_field(obj, pan) or []
            field = ".".join(path)

            # record for Excel
            matches_data.append((identifier, ts, payload, pan, field))
    ...
    result_list.append((matches_data, seen, with_pan))
...
def write_excel(all_matches):
    ...
    ws.write_row(0, 0, ["Identifier", "Timestamp", "Payload", "PAN", "Field"])
    r = 1
    for idf, ts, payload, pan, field in all_matches:
        ws.write(r, 0, idf)
        ws.write(r, 1, ts)
        ws.write(r, 2, payload)
        ws.write(r, 3, pan, red)
        ws.write(r, 4, field)
        r += 1
    wb.close()
...
if __name__ == "__main__":
    ...
    print("=== PAN EXTRACTOR w/ FIELD NAMES ===")
    ...
    all_matches = []
    tot = has_pan = 0
    for data, seen, with_pan in results:
        all_matches.extend(data)
        tot += seen
        has_pan += with_pan

    print(f"Records scanned: {tot}, with PAN: {has_pan}, matches: {len(all_matches)}")
